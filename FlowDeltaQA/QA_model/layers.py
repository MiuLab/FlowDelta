...
class FlowAttentionScore(nn.Module):
    ...
    """
    def __init__(self, x1_input_size, x2_input_size, attention_hidden_size, similarity_score = False, tight_weight=True):
        super(FlowAttentionScore, self).__init__()
        self.tight_weight = tight_weight
        if tight_weight:
            assert x1_input_size == x2_input_size
            self.linear = nn.Linear(x1_input_size, attention_hidden_size, bias=False)
        else:
            self.linear1 = nn.Linear(x1_input_size, attention_hidden_size, bias=False)
            self.linear2 = nn.Linear(x2_input_size, attention_hidden_size, bias=False)
...
class FlowAttn(nn.Module):
    def __init__(self, x1_input_size, x2_input_size, attention_hidden_size, similarity_attention = False, tight_weight=True):
        super(FlowAttn, self).__init__()
        self.scoring = FlowAttentionScore(x1_input_size, x2_input_size, attention_hidden_size, similarity_score=similarity_attention, tight_weight=tight_weight)

    def forward(self, x1, x2, x3=None, scores=None, return_scores=False, drop_diagonal=False):
        """
        ...
        Return:
            matched_seq: batch * len1 * x3_input_size
        """
        if x3 is None:
            x3 = x2
