...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, context_feature=None):
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, flow=None):
        attention_output = self.attention(hidden_states, attention_mask)
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output, flow)
        return layer_output
...
class BertModel(PreTrainedBertModel):
    ...
    def __init__(self, config, prv_ctx=2, use_flow=True):
        ...
        self.embeddings = BertEmbeddings(config, prv_ctx)
        self.encoder = BertEncoder(config, use_flow)
        self.pooler = BertPooler(config)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, context_feature=None, output_all_encoded_layers=True):
        ...
        embedding_output = self.embeddings(input_ids, token_type_ids, context_feature)
...
class BertForQuestionAnsweringExtend(PreTrainedBertModel):
    """BERT model for CoQA (span extraction).
    ...
    def __init__(self, config, class_num, prv_ctx):
        ...
        self.slot_logits_mlp = nn.Linear(config.hidden_size * 2, class_num*2)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, context_feature=None, start_positions=None, end_positions=None):
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask, context_feature, output_all_encoded_layers=False)
        
        flow_in = sequence_output.permute(1, 0, 2)
        ...
        slot_logits = self.slot_logits_mlp(first_token_tensor)
        slot_logits = slot_logits.view(-1, self.class_num, 2)

        final_logits = torch.cat([slot_logits, logits[:, 1:, :]], 1)

        start_logits = final_logits[:, :, 0]
        end_logits = final_logits[:, :, 1]
...
class BertForQuestionAnswering(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, context_feature=None, start_positions=None, end_positions=None, is_impossible=None):
        sequence_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, context_feature, output_all_encoded_layers=False)
